...
def api_delete_backup(backup_id):
    ...
    try:
        ...
        ok, msg = transfer_manager.delete_backup_options(backup_id, delete_record=delete_record, delete_files=delete_files)
        if ok:
            return jsonify({"status": "success", "message": msg})
        else:
            return jsonify({"status": "error", "message": msg}), 400
...
@app.route('/api/backups/<backup_id>/plan', methods=['POST'])
def api_plan_backup_restore(backup_id):
    ...
    try:
        ...
        if files and not isinstance(files, list):
            return jsonify({"status": "error", "message": "'files' must be a list of relative paths"}), 400
        plan = transfer_manager.plan_context_restore(backup_id, files)

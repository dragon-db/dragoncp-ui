...
| [`reference/api.md`](reference/api.md) | The hand-written HTTP reference and the declared authority on the API — every `/api/*` endpoint across eleven sections with request/response bodies, status codes, the auth model, and a closing checklist counting 85 method+path endpoints verified against the route decorators. |
| [`reference/openapi.yaml`](reference/openapi.yaml) | The machine-readable companion to `api.md`, for importing into an API client or generating a stub client: 85 operations with method, path, auth requirement and a one-line summary, and deliberately thin schemas. Both files are maintained by hand; when they disagree, `api.md` wins and this file is the one to fix. |
...
6. **`api.md` and `openapi.yaml` agree today by hand, and only by hand.** Both now describe 85 operations, but they are maintained as two independent hand-written files with no generator and no check. The next endpoint change is free to desynchronise them silently.

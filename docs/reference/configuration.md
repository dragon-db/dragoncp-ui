...
| `CORS_ORIGINS` | Comma-separated allowed origins for the React frontend and Socket.IO (`app.py:93-100`, `:184-192`, `:284-301`). `*` allows everything. | `*` | List every origin the app is reached on, **including its own address** — Flask serves the built React app, so same-origin is the normal case, and setting a list replaces Socket.IO's default of accepting its own origin. A missing origin does not fail like an ordinary CORS error: the Socket.IO handshake succeeds (browsers omit `Origin` on a same-origin GET) and the next request is rejected with a 400 (they send it on every POST), so the socket connects and drops in a permanent reconnect loop while ordinary API GETs keep working. A different port, hostname or scheme is a different origin. Note that with `*` no credentials header is sent. |

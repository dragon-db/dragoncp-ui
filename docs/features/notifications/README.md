...
- **Backup restores never notify.** Restore operations create their own transfer rows inside `services/backup_service.py` without going through `start_transfer()`, so they have no completion watcher.

...
class DragonCPUI {
    ...
    renderBackupFilesInline(files) {
        ...
        files.forEach(f => {
            ...
            tr.innerHTML = `
                ...
                <td><code>${this.escapeHtml(f.context_display || '')}</code></td>
    ...
    async restoreBackup(backupId, files = null) {
        try {
            ...
            if (planData.status !== 'success') {
                this.showAlert(planData.message || 'Failed to plan restore', 'danger');
                return;
            }
    ...
    renderDeleteFilesPreview(files) {
        ...
        files.forEach(file => {
            ...
            tr.innerHTML = `
                <td>
                    <div><strong>Backup file:</strong> <code>${this.escapeHtml(file.relative_path || 'Unknown')}</code></div>
                    <div class="text-muted small"><i class="bi bi-folder"></i> ${this.escapeHtml(backupFilePath)}</div>
                </td>
                <td class="text-center">
                    ${file.file_size ? this.humanReadableBytes(file.file_size) : 'Unknown'}
                </td>
            `;
            tbody.appendChild(tr);
        });
    ...
    async executeDelete() {
        try {
            ...
            if (!deleteRecordCheck || !deleteFilesCheck) {
                this.showAlert('Delete options not found', 'danger');
                return;
            }
            ...
            if (!deleteRecord && !deleteFiles) {
                this.showAlert('Please select at least one delete option', 'warning');
                return;
            }
            ...
            if (!backupId) {
                this.showAlert('No backup selected for deletion', 'danger');
                return;
            }
            ...
            if (data.status === 'success') {
                this.showAlert(data.message || 'Delete completed successfully', 'success');
                this.loadBackups(); // Refresh the list
                this.showBackupsListStage(); // Return to main view
            } else {
                this.showAlert(data.message || 'Delete failed', 'danger');
            }
        ...
        try {
            ...
            ops.forEach(op => {
                ...
                if (op.context_display && op.context_display !== op.backup_relative) {
                    contextHtml = `<div class="text-info small"><i class="bi bi-tag"></i> ${this.escapeHtml(op.context_display)}</div>`;
                } else {
                ...
                if (op.target_delete && op.target_delete !== op.copy_to) {
                    actionHtml = `
                        <div class="text-warning small">
                            <i class="bi bi-arrow-repeat"></i> Replace: <code>${this.escapeHtml(op.target_delete)}</code>
                        </div>
                        <div class="text-success small">
                            <i class="bi bi-plus-circle"></i> Copy to: <code>${this.escapeHtml(op.copy_to)}</code>
                        </div>
                    `;
                } else {
                    actionHtml = `
                        <div class="text-success small">
                            <i class="bi bi-plus-circle"></i> New file: <code>${this.escapeHtml(op.copy_to)}</code>
                        </div>
                    `;
                }
                ...
                tr.innerHTML = `
                    <td>
                        <div><strong><code>${this.escapeHtml(op.backup_relative)}</code></strong></div>
                        ${contextHtml}
                    </td>
                    <td>
                        ${actionHtml}
    ...
    async applyPlannedRestore() {
        try {
            ...
            this.showAlert('Apply restore failed', 'danger');
        }
    }
